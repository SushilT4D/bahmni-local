...
--
-- Take a backup first (pg_dump --format=custom, verified with pg_restore -l).
--
-- VERIFY THROUGH THE APPLICATION, not by counting rows: the lab catalogue must
-- offer its tests for a sample type (116 for sample type 84 on the seed), which
-- fails with "column test0_.reference_info does not exist" until these apply;
-- login and accession lookup must still pass afterwards.
--
...
-- 'groupBySample', matching BHS production. It does NOT partition accession
-- numbers per site -- that is acessionFormat (DATENUM, no site component) and
-- siteNumber (11404 on every node), neither of which these changesets touch.
...
CREATE OR REPLACE FUNCTION pg_temp.mark(p_id text, p_author text, p_comment text) RETURNS void AS $f$
  ...
  VALUES (p_id, p_author, './Bahmni/BahmniConfig.xml', now(), NULL, 'Custom SQL',
          p_comment || ' [applied by clinic/openelis/apply-missing-changesets.sql]', '1.9.5');
$f$ LANGUAGE sql;
