SELECT 'Total number of clinic visits' as Indicator, round(count(*)) AS 'Morwal'
  ...
  INNER join location l on l.location_id=v.location_id and l.name like '%Morwal%'

union all

SELECT 'Total no. of patients' as Indicator,round(count(distinct pi.identifier)) AS 'Morwal'
  FROM patient_identifier pi
  JOIN person p ON p.person_id = pi.patient_id AND pi.voided=0 AND pi.preferred=1
  INNER join visit v on v.patient_id=p.person_id AND v.voided=0 and
  cast(v.date_started AS DATE) between '#startDate#' AND '#endDate#'
...
select 'Number of female %' as Indicator, concat(Female_patient,' (',round((Female_patient / Total_visit)*100,2),')') as Morwal from (
SELECT 
...
join location l on l.location_id = v.location_id and l.name like '%Morwal%'
  ) as Total_visit
  FROM patient_identifier pi 
  ...
  INNER join location l on l.location_id=v.location_id and l.name like'%Morwal%'
  order by v.date_created ) as Morwal_visit

union all

select 'Number of children under 5 years (%)' as Indicator, concat(cpatient,' (',round((cpatient / Total_visit)*100,2),')') as Morwal from (
SELECT 
...
union all
 ...
 select 'Number of girls (%)' as Indicator,concat(Female_patient,' (',round((Female_patient / Total_visit)*100,2),')') as Morwal from (
SELECT 
count(distinct(pi.identifier)) as Female_patient, 
(
...
SELECT 'No. of new patients registered' as Indicator,round(count(distinct pi.identifier))  AS 'Morwal'
  FROM patient_identifier pi 
  JOIN person p ON p.person_id = pi.patient_id AND pi.voided=0 AND pi.preferred=1 
  and cast(p.date_created AS DATE) between '#startDate#' AND '#endDate#' and pi.identifier like '%MOR%'
...
FROM obs o
  ...
  WHERE o.voided is false and l.name like '%Morwal%'
...
join person p on p.person_id = pi.patient_id and pi.voided = 0 and pi.preferred = 1
join visit v on v.patient_id = p.person_id and v.voided = 0 and cast(v.date_started AS DATE) between '#startDate#' AND '#endDate#'
join location l on l.location_id = v.location_id and l.name like '%Morwal%'))*100,2),')'
) as Morwal
  FROM patient_identifier pi 
  JOIN person p ON p.person_id = pi.patient_id AND pi.voided=0 AND pi.preferred=1
  LEFT OUTER JOIN person_address paddress ON p.person_id = paddress.person_id AND paddress.voided=0 
...
round(count(distinct(pi.identifier))) as Morwal
FROM obs o
  ...
  and cast(v.date_started AS DATE) between '#startDate#' AND '#endDate#'
  JOIN visit_type vt ON vt.visit_type_id=v.visit_type_id AND vt.retired is false
  JOIN location l ON e.location_id = l.location_id AND l.retired is false and l.name like'%Morwal%'
  LEFT JOIN obs parent_obs ON parent_obs.obs_id=o.obs_group_id
  LEFT JOIN concept_name parent_cn ON parent_cn.concept_id=parent_obs.concept_id AND parent_cn.concept_name_type="FULLY_SPECIFIED"
  LEFT JOIN concept_name coded_fscn on coded_fscn.concept_id = o.value_coded AND coded_fscn.concept_name_type="FULLY_SPECIFIED" AND coded_fscn.voided is false
  LEFT JOIN concept_name coded_scn on coded_scn.concept_id = o.value_coded AND coded_scn.concept_name_type="SHORT" AND coded_scn.voided is false
...
union all
 ...
 select 'No of ANC with high risk identified' as Indicator, round(count(distinct(formquery.Identifier))) as Morwal from 
 (
    SELECT
...
select 'Total revenue generated in rupees' as Indicator,round(sum(o.value_numeric)) as Morwal 
from obs o 
...
Inner JOIN person p ON p.person_id = o.person_id AND p.voided is false 
 INNER join location l on l.location_id=o.location_id and l.name like '%Morwal%'
