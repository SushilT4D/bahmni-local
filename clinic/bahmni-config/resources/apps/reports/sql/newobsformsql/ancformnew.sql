...
GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'ANC_other_diagnosis', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Other-Diagnosis', 
...
FROM obs o
    ...
    and cast(o.obs_datetime AS DATE) BETWEEN '#startDate#'AND '#endDate#'
     ...
     AND obs_fscn.name IN (
       'Location of Visit','Phulwari_Text','OutReach_Text','ANC_No_Of_Visit','ANC_Gravida','ANC_Parity','ANC_Abortions','ANC_Live_Births','ANC_Still_Births','ANC_Neonatal_Deaths','ANC_LMP','ANC_EDD','ANC-WEIGHT',
       'ANC_HEIGHT','ANC_BMI','ANC_Hemoglobin','HIV Status new','ANC_High_risk','ANC_Any_Complications','ANC_Any_Other_Problems','ANC_other_diagnosis','Patient referred_institute','RF_Institute_Name','GLOBAL_Patient_Admitted','Consulted on Call','Receipt_number',
       'Fees_Amount','Subscription card','Subscription card Numbers','GLOBAL_Doctors_Name','GLOBAL_Nurse_Name'
        )
