...
GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'HIV Status new', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'HIV Status',
GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'ACS_ADULT_Diagnosis', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Diagnosis',
GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'ACS_other_diagnosis', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Other-Diagnosis',
GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'ACS_Sever_Illness', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Severe illness',
GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'Patient referred_institute', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Patient referred',
...
(
 ...
 paddress.state_province as 'State',
  ...
  (
            ...
            AND pa.person_id = p.person_id and cn.concept_name_type='SHORT'
            LIMIT 1 
        ) As 'Migrant'      FROM obs o
    JOIN concept obs_concept ON obs_concept.concept_id=o.concept_id AND obs_concept.retired is false  and o.form_namespace_and_path like '%Adult Case Sheet%'
    ...
    AND obs_fscn.name IN ('GLOBAL_Type_Of_Visit','Location of Visit','Phulwari_Text','OutReach_Text','Global_weight','Global_height','BMI','Lab_Hemoglobin','HIV Status new','ACS_ADULT_Diagnosis','ACS_other_diagnosis','ACS_Sever_Illness','Patient referred_institute','RF_Institute_Name','GLOBAL_Patient_Admitted','Consulted on Call',
      'Receipt_number','Fees_Amount','Subscription card','Subscription card Numbers','GLOBAL_Doctors_Name','GLOBAL_Nurse_Name')
