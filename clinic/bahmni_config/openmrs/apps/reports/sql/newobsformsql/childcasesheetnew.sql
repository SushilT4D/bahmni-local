...
GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'CCS_other_diagnosis', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Other-Diagnosis',
...
(
 ...
 paddress.state_province as 'State',
  ...
  paddress.postal_code  as 'Hamlet (Fala)'
     FROM obs o
    JOIN concept obs_concept ON obs_concept.concept_id=o.concept_id AND obs_concept.retired is false  and o.form_namespace_and_path like '%Child Case Sheet%'
    ...
    AND obs_fscn.name IN ('Subscription card Numbers','Subscription card','Consulted on Call','RF_Institute_Name','GLOBAL_Type_Of_Visit','Location of Visit','HIV Status new','CCS_other_diagnosis','Phulwari_Text','OutReach_Text','GLOBAL_Infant_Weight','GLOBAL_Infant_Height','Lab_Hemoglobin','WAZ_Color','WHZ','2MT5Y_Diagnosis','2M_Serious_Illness',
        'Patient referred_institute','GLOBAL_Patient_Admitted','Receipt_number','Fees_Amount','GLOBAL_Doctors_Name','GLOBAL_Nurse_Name')
    ...
    WHERE o.voided is false AND o.obs_datetime BETWEEN '2024-07-01' AND CURRENT_DATE()
