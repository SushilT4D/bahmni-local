...
# a rerun must not mistake the node's own MirrorMaker for a twin
blk="$(sed -n '/# own-mm2:begin/,/# own-mm2:end/p' "$T90")"
[ -n "$blk" ] || bad "090 has no own-mm2 block"
printf '%s' "$blk" | grep -q "inspect --format '{{.State.Running}}' mirrormaker-connect" && ok_ "the guard looks at the node's own mirrormaker-connect first" || bad "the guard does not look at the node's own MirrorMaker"
printf '%s' "$blk" | grep -q 'stop mirrormaker-connect' && ok_ "the guard stops the node's own MirrorMaker before reading heartbeats" || bad "the guard does not stop the node's own MirrorMaker"
awk '/# own-mm2:end/{f=1} f&&/hb_read\(\)/{print "order-ok"; exit}' "$T90" | grep -q order-ok && ok_ "the stop comes before the heartbeat reads" || bad "the stop is not before the reads"
