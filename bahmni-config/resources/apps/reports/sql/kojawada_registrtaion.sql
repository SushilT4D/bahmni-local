SELECT  pi.identifier AS 'Patient Identifier',
  ...
  (
            ...
            AND pa.person_id = p.person_id and cn.concept_name_type='FULLY_SPECIFIED'
            LIMIT 1 
        ) as 'Occupation',
  (
            SELECT cn.name
            FROM concept_name cn
            JOIN person_attribute pa ON cn.concept_id = pa.value
            WHERE pa.person_attribute_type_id = 33
            AND pa.person_id = p.person_id and cn.concept_name_type="FULLY_SPECIFIED"
            LIMIT 1 
        ) as 'File Rack Number',
  (
            ...
            FROM concept_name cn
            JOIN person_attribute pa ON cn.concept_id = pa.value
            WHERE pa.person_attribute_type_id = 34
            AND pa.person_id = p.person_id and cn.concept_name_type="FULLY_SPECIFIED"
            LIMIT 1 
        ) as 'Family Number',
  'Kojawada' as 'Location'
  FROM patient_identifier pi
  JOIN person p ON p.person_id = pi.patient_id AND pi.voided=0 AND pi.preferred=1 and pi.identifier like '%KOJ%'
